...
Generate the "PR by author" list for cvxpylayers release notes.

Compares the release tag against the previous release branch so that
cherry-picked patch PRs are not duplicated.

Usage:
    python tools/release_notes.py v1.2.0 release/1.1.x
    python tools/release_notes.py v1.2.0  # auto-detects previous release branch
"""
...
def parse_tag(tag: str) -> tuple[int, int, int]:
    """Parse a tag like v1.2.0 into (major, minor, micro)."""
...
def get_previous_release_branch(tag: str) -> str:
    """Infer the previous release branch from a tag like v1.2.0."""
    major, minor, _ = parse_tag(tag)
    return f"origin/release/{major}.{minor - 1}.x"


def get_previous_release_tag(tag: str) -> str:
    """Infer the previous minor release tag from a tag like v1.2.0."""
    major, minor, _ = parse_tag(tag)
...
def main():
    if len(sys.argv) < 2:
        ...
        print(f"  e.g. {sys.argv[0]} v1.2.0  (minor release)")
        print(f"  e.g. {sys.argv[0]} v1.1.5  (patch release)")
        sys.exit(1)
    ...
    for i in range(0, len(pr_numbers), 100):
        ...
        query = "{ repository(owner: \"cvxpy\", name: \"cvxpylayers\") { " + " ".join(parts) + " } }"
        try:
