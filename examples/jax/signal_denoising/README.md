...
where `y` is the denoised output. Both the weighting matrix `theta` (n x n) and regularization strength `lambda` are learned end-to-end via Adam (optax) to minimize MSE on noisy cosine signals.
