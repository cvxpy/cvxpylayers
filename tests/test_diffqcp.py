...
def compare_solvers(problem, params, param_vals, variables):
    ...
    # Test CuClarabel
    layer_cuclarabel = CvxpyLayer(problem, params, variables, solver="DIFFQCP")
...
def test_mixed_batched_unbatched():
    ...
    # Test CuClarabel
    layer_cuclarabel = CvxpyLayer(problem, [A, b], [x], solver="DIFFQCP")
...
def test_batch_size_one_preserves_batch_dimension():
    ...
    layer_cuclarabel = CvxpyLayer(problem, parameters=[b], variables=[x], solver="DIFFQCP")
...
def test_sdp_rejected():
    ...
    with pytest.raises(SolverError, match="do not support.*PSD"):
        CvxpyLayer(problem, [], [X], solver="DIFFQCP")
...
def test_jax_interface_forward_pass():
    ...
    layer = JaxCvxpyLayer(problem, [A, b, G, h], [x], solver="DIFFQCP")
...
def test_jax_interface_batched():
    ...
    # Test JAX interface with CuClarabel
    layer = JaxCvxpyLayer(problem, [A, b], [x], solver="DIFFQCP")
