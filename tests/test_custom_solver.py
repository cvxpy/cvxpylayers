...
Three solver implementations are tested:
  ...
  (c) SolverInterface.from_parametric_functions() — wraps a CVXPYgen-generated
      C solver (requires cvxpygen + cmake; skipped when not available)

Also tests:
  (d) SolverInterface.from_functions() — functional-wrapping factory
...
# ===========================================================================
# Tests: SolverInterface.from_functions() — functional-wrapping factory
# ===========================================================================

def test_from_functions_forward(problem_and_ref):
    """from_functions() wrapping a pair of lambdas matches the reference."""
    n, m = 2, 3
    problem, A_param, b_param, x_var, ref = problem_and_ref
    mock = NumpySingleMock(ref.ctx.solver_ctx)

    solver = SolverInterface.from_functions(
        solve      = mock.solve_numpy,
        derivative = mock.derivative_numpy,
    )
    layer = CvxpyLayer(problem, parameters=[A_param, b_param],
                       variables=[x_var], solver=solver)
...
def test_from_functions_backward(problem_and_ref):
    """from_functions() gradient matches the reference."""
    n, m = 2, 3
    problem, A_param, b_param, x_var, ref = problem_and_ref
    mock = NumpySingleMock(ref.ctx.solver_ctx)
