...
class CUCLARABEL_ctx:
    ...
    def __init__(
        ...
        self.nnz_A = nnz_A
        self.b_idxs = jnp.array(b_idx)

        self.A_structure = jnp.array(A_csr_structure[0]), jnp.array(A_csr_structure[1])

        if P_csr_structure is not None:
            self.P_shape = P_shape
            self.P_structure = jnp.array(P_csr_structure[0]), jnp.array(P_csr_structure[1])
        else:
            self.P_shape = (A_shape[1], A_shape[1])
            self.P_structure = (jnp.array([], dtype=int), jnp.array([0] * (A_shape[1] + 1)))
