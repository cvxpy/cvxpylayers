...
class MPAX_ctx:
    ...
    def __init__(
        ...
        self.Q_structure = P_csr_structure
        self.Q_shape = P_shape

        # RHS handling: b_idx entries after the nnz_A values
        self.last_col_start = nnz_A
        self.last_col_end = nnz_A + len(b_idx)
        self.last_col_indices = b_idx
        self.m = m

        # Split constraint CSR at dims.zero into equality (A) and inequality (G)
        col_indices, row_offsets = A_csr_structure
        split = row_offsets[dims.zero]
        self.nnz_A_eq = split
        self.nnz_G = nnz_A - split
